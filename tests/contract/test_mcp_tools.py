...
def test_mcp_search_by_technology_scopes_hits(tmp_path: Path) -> None:
    root = tmp_path / "ws"
    _seed(root)
    server = create_mcp_server(root)
    result = _call(
        ...
        {"technology": "technology:oracle", "text": "ORA-12541", "limit": 10},
    )
    assert result.is_error is False
    payload = result.structured_content
    assert payload["schema_version"] == MCP_SCHEMA_VERSION
    assert len(payload["documents"]) >= 1
    assert all(item["document"]["technology"] == "oracle" for item in payload["documents"])
    titles = {item["document"]["title"] for item in payload["documents"]}
    assert "Oracle listener" in titles


def test_mcp_search_by_technology_empty_slug(tmp_path: Path) -> None:
    root = tmp_path / "ws"
    _seed(root)
    server = create_mcp_server(root)
    result = _call(server, "search_by_technology", {"technology": "technology:", "limit": 10})
    assert result.is_error is False
    payload = result.structured_content
    assert payload["schema_version"] == MCP_SCHEMA_VERSION
    assert payload["documents"] == []
...
def test_mcp_search_by_tag_scopes_hits(tmp_path: Path) -> None:
    root = tmp_path / "ws"
    _seed(root)
    server = create_mcp_server(root)
    result = _call(
        server,
        "search_by_tag",
        {"tag": "tag:oracle", "text": "ORA-12541", "limit": 10},
    )
    assert result.is_error is False
    payload = result.structured_content
    assert payload["schema_version"] == MCP_SCHEMA_VERSION
    assert len(payload["documents"]) >= 1
    assert all("oracle" in item["document"]["tags"] for item in payload["documents"])
    titles = {item["document"]["title"] for item in payload["documents"]}
    assert "Oracle listener" in titles or "Listener tip" in titles


def test_mcp_search_by_tag_empty_slug(tmp_path: Path) -> None:
    root = tmp_path / "ws"
    _seed(root)
    server = create_mcp_server(root)
    result = _call(server, "search_by_tag", {"tag": "tag:", "limit": 10})
    assert result.is_error is False
    payload = result.structured_content
    assert payload["schema_version"] == MCP_SCHEMA_VERSION
    assert payload["documents"] == []
...
def test_mcp_list_facets_discovers_slugs(tmp_path: Path) -> None:
    root = tmp_path / "ws"
    _seed(root)
    server = create_mcp_server(root)
    result = _call(server, "list_facets", {})
    assert result.is_error is False
    payload = result.structured_content
    assert payload["schema_version"] == MCP_SCHEMA_VERSION
    facets = payload["facets"]
    projects = {item["value"] for item in facets["projects"]}
    assert "payments" in projects
    components = {item["value"] for item in facets["components"]}
    assert "listener" in components
    technologies = {item["value"] for item in facets["technologies"]}
    assert "oracle" in technologies
    tags = {item["value"] for item in facets["tags"]}
    assert "oracle" in tags
    source_types = {item["value"] for item in facets["source_types"]}
